...
use App\Helpers\General\MoneyToStr;
...
if (! function_exists('money_to_str')) {
	/**
	 * Convert money to string.
	 */
	function money_to_str($value)
	{
		$moneyToStrUAH = new MoneyToStr("UAH", "UKR", "NUMBER");
		return $moneyToStrUAH->convertValue($value);
	}
}
...
if (! function_exists('home_route')) {
	...
	function home_route()
	{
		if (auth()->check()) {
			...
			} elseif (auth()->user()->can('view customer')) {
				return 'customer.dashboard';
			} elseif (auth()->user()->can('view manager')) {
				return 'manager.dashboard';
			} elseif (auth()->user()->can('view accountant')) {
				return 'accountant.dashboard';
			} elseif (auth()->user()->can('view dtp')) {
...
// Uppercase first letter of invoice sum
